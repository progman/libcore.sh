...
# make options
	...
	OPT+=" --label FLAG_DEBUG=${FLAG_DEBUG}";
	OPT+=" --build-arg FLAG_DEBUG=${FLAG_DEBUG}";

# build
	echo "DOCKER_BUILDKIT=1 docker build${OPT} ./;"
	if [ "${FLAG_DEBUG}" != "1" ];
	then
		DOCKER_BUILDKIT=1 docker build${OPT} ./ &> /dev/null < /dev/null
	else
		DOCKER_BUILDKIT=1 docker build${OPT} ./
	fi
	if [ "${?}" != "0" ];
	then
...
# pull
	local CMD="";
	CMD+="docker compose";
	CMD+=" --project-name ${DOCKER_PROJECT_NAME}";
...
function help()
{
	echo "example: ${1} [ registry | login | build, b | pull | push DOCKER_IMAGE | flush, f | ps | test, t | deploy | up, u | down, d | reup, r | gc | gc_log | gc_total ]";
}
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
# general function
function main()
