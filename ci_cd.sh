...
# fetch
		FETCH_LIST_INDEX=0;
		FETCH_LIST_COUNT=$(cat ${CI_CD_CONFIG} | jq "${TEMPLATE_PROJECT}.fetch_list | length");
		while true;
		do
#			echo "=====================================================================================================================";
			TEMPLATE_FETCH="${TEMPLATE_PROJECT}.fetch_list[${FETCH_LIST_INDEX}]";
...
# deploy
		DEPLOY_LIST_INDEX=0;
		DEPLOY_LIST_COUNT=$(cat ${CI_CD_CONFIG} | jq "${TEMPLATE_PROJECT}.deploy_list | length");
		while true;
		do
#			echo "=====================================================================================================================";
			TEMPLATE_DEPLOY="${TEMPLATE_PROJECT}.deploy_list[${DEPLOY_LIST_INDEX}]";
...
#			echo "DEPLOY_DIR:${DEPLOY_DIR}";
		...
		(( PROJECT_LIST_INDEX++ ));
		if [ ${PROJECT_LIST_INDEX} -eq ${PROJECT_LIST_COUNT} ];
		then
			break;
		fi
	done
